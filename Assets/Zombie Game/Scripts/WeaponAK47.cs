...
public class WeaponAK47 : MonoBehaviour {
	public GameObject bullet, muzzleFlashObj, bulletSpawnPoint, bulletShell, shellSpawnPoint;
	...
	public bool Shoot () {
		if (lastShot <= 0.0f) {
			...
			if (lastAccurateShot > 0.0f) {
				tempDirection.x += Random.value/40*Random.Range(-1.0f,1.0f);
				tempDirection.y += Random.value/40*Random.Range(-1.0f,1.0f);
			}
